...
using namespace std;
// GL Includes
#include <GL/gl3w.h> // Contains all the necessery OpenGL includes
#include <glm/glm.hpp>
...
private:
	...
	Mesh processMesh(aiMesh* mesh, const aiScene* scene)
	{
		...
		for (GLuint i = 0; i < mesh->mNumVertices; i++)
		{
			...
			// Tangent
			if (mesh->mTangents) // Does the mesh contain tangent coordinates?
			{
			...
			// Bitangent
			if (mesh->mBitangents) // Does the mesh contain bitangent coordinates?
			{
